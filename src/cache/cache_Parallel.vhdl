...
-- Upon reading a cache line, the current content is outputted on ``CacheLineOut``
-- with a latency of one clock cycle.
--
-- Upon replacing a cache line, the new content is given by ``CacheLineIn``. The
-- old content is outputted on ``CacheLineOut`` and the old tag on ``OldAddress``,
...
architecture rtl of cache_Parallel is
