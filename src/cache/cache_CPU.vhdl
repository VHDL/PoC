...
-- directly fed by the address calculator. But be aware of the high setup time
...
entity cache_CPU is
	...
	port (
		...
		cpu_Address   : in  unsigned(log2ceil(MEM_DATA_BITS/CPU_DATA_BITS)+MEM_ADDR_BITS-1 downto 0);
		cpu_WriteData : in  std_logic_vector(CPU_DATA_BITS-1 downto 0);
		cpu_WriteMask : in  std_logic_vector(CPU_DATA_BITS/8-1 downto 0);
		cpu_Got       : out std_logic;
		cpu_ReadData  : out std_logic_vector(CPU_DATA_BITS-1 downto 0);

		-- Memory side
		mem_Request     : out std_logic;
		mem_Write      : out std_logic;
		mem_Address     : out unsigned(MEM_ADDR_BITS-1 downto 0);
		mem_WriteData  : out std_logic_vector(MEM_DATA_BITS-1 downto 0);
		mem_WriteMask  : out std_logic_vector(MEM_DATA_BITS/8-1 downto 0);
		mem_Ready       : in  std_logic;
		mem_ReadStrobe : in  std_logic;
		mem_ReadData   : in  std_logic_vector(MEM_DATA_BITS-1 downto 0)
		);
end entity;
...
begin  -- architecture rtl
	...
	cache_inst: entity work.cache_Parallel2
		generic map (
	...
	gEqual: if RATIO = 1 generate -- Cache line size equals CPU data bus size
		cpu_wdata_wide <= cpu_WriteData;
		cpu_wmask_wide <= cpu_WriteMask;
		cpu_ReadData      <= cache_LineOut;
	end generate gEqual;
	...
	begin
		...
		lower_addr <= cpu_Address(LOWER_ADDR_BITS-1 downto 0);

		l0: for i in 0 to RATIO-1 generate
			cpu_wdata_wide((i+1)*CPU_DATA_BITS-1 downto i*CPU_DATA_BITS) <= cpu_WriteData;

			cpu_wmask_wide((i+1)*CPU_DATA_BITS/8-1 downto i*CPU_DATA_BITS/8) <=
				-- synthesis translate_off
				(others => 'X') when is_x(lower_addr) else
				-- synthesis translate_on
				cpu_WriteMask when to_integer(lower_addr) = i else
				(others => '1');
		end generate l0;

		-- CPU Reply Data Path
		lower_addr_r <= lower_addr when rising_edge(Clock); -- pipeline register
	...
	cache_Address   <= std_logic_vector(cpu_Address(cpu_Address'left downto LOWER_ADDR_BITS));
	cache_LineIn    <= mem_ReadData       when fsm_cs = READING_MEM else cpu_wdata_wide;
	...
	mem_Write <= cpu_Write;
	mem_Address  <= cpu_Address(cpu_Address'left downto LOWER_ADDR_BITS);
	mem_WriteData <= cpu_wdata_wide;
	mem_WriteMask <= cpu_wmask_wide;

	-- FSM
	-- ===========================================================================
	process(fsm_cs, cpu_Request, cpu_Write, cache_Hit, cache_Miss, mem_Ready, mem_ReadStrobe)
	begin
		...
		case fsm_cs is
			when READY =>
				...
				-- -----------------------------
				cache_Request     <= to_x01(cpu_Request);
