...
begin
	...
	cache_CPU_inst: entity work.cache_CPU
		...
		port map (
			...
			cpu_Request   => int_req,
			cpu_Write => int_write,
			cpu_Address  => int_addr,
			cpu_WriteData => int_wdata,
			cpu_WriteMask => int_wmask,
			cpu_Got   => int_got,
			cpu_ReadData => int_rdata,
			mem_Request   => mem_req,
			mem_Write => mem_write,
			mem_Address  => mem_addr,
			mem_WriteData => mem_wdata,
			mem_WriteMask => mem_wmask,
			mem_Ready   => mem_rdy,
			mem_ReadStrobe  => mem_rstb,
			mem_ReadData => mem_rdata);

	-- read data is valid one clock cycle after int_got is asserted
	cpu_rstb  <= (not Reset) and (not int_write) and int_got when rising_edge(Clock);
