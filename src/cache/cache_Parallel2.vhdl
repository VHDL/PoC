...
begin
	...
	-- Cache TagUnit
	TU: entity work.cache_TagUnit_Parallel
		generic map (
	...
	begin
		...
		data_mem: entity work.ocram_SinglePort
			generic map (
				ADDRESS_BITS   => LINE_INDEX_BITS,
				DATA_BITS   => 8, -- 8 bit per lane
				FILENAME => "")
			port map (
				Clock => Clock,
				ClockEnable  => MemoryAccess,
				WriteEnable  => we,
				Address   => MemoryIndex_us,
				DataIn   => CacheLineIn (i*8+7 downto i*8),
				DataOut   => CacheLineOut(i*8+7 downto i*8));
	end generate gLane;
