...
package body axi4_Full is
	...
	function AddressMask(InBus : T_AXI4_Bus_M2S; Mask : std_logic_vector) return T_AXI4_Bus_M2S is
		variable temp : InBus'subtype;
	begin
		assert Mask'length = InBus.AWAddr'length report "PoC.axi4_Full.AddressMask: Length of Mask-Bits and Address-Bits is no equal!" severity failure;
	...
	function IDResize(InBus : T_AXI4_Bus_M2S; AWIDBits : natural; ARIDBits : natural) return T_AXI4_Bus_M2S is
		variable temp : T_AXI4_Bus_M2S(
		AWID(AWIDBits -1 downto 0), ARID(ARIDBits -1 downto 0),
		AWUser(InBus.AWUser'range), ARUser(InBus.ARUser'range), WUser(InBus.WUser'range),
		WData(InBus.WData'range), WStrb(InBus.WStrb'range),
		AWAddr(InBus.AWAddr'range), ARAddr(InBus.ARAddr'range)
		);
	begin
		temp.AWID     := resize(InBus.AWID, AWIDBits);
		temp.AWAddr   := InBus.AWAddr;
		temp.AWLen    := InBus.AWLen;
		temp.AWSize   := InBus.AWSize;
		temp.AWBurst  := InBus.AWBurst;
		...
		temp.ARCache  := InBus.ARCache;
		temp.ARProt   := InBus.ARProt;
		temp.ARID     := resize(InBus.ARID, ARIDBits);
		temp.ARLen    := InBus.ARLen;
		temp.ARSize   := InBus.ARSize;
		temp.ARBurst  := InBus.ARBurst;
		temp.ARLock   := InBus.ARLock;
	...
	function IDResize(InBus : T_AXI4_Bus_S2M; RIDBits : natural; BIDBits : natural) return T_AXI4_Bus_S2M is
		variable temp : T_AXI4_Bus_S2M(
		BID(BIDBits -1 downto 0), RID(RIDBits -1 downto 0),
		BUser(InBus.BUser'range), RUser(InBus.RUser'range),
		RData(InBus.RData'range)
		);
	begin
