...
architecture rtl of axi4_FIFO is
	...
	constant W_Addr_BIT_VEC   : positive_vector := ( ID_POS    => In_M2S.AWID'length,    User_POS    => In_M2S.AWUser'length,    Addr_POS => In_M2S.AWAddr'length,
	...
	constant R_Addr_BIT_VEC   : positive_vector := ( ID_POS    => In_M2S.ARID'length,    User_POS    => In_M2S.ARUser'length,    Addr_POS => In_M2S.ARAddr'length,
	...
	constant W_BIT_VEC      : positive_vector := (User_POS => In_M2S.WUser'length, Strobe_POS => In_M2S.WStrb'length, Data_POS => In_M2S.WData'length,
																				 Last_POS => LAST_BITS
		);
	constant R_BIT_VEC      : positive_vector := (ID_POS   => In_S2M.RID'length,   User_POS => In_S2M.RUser'length, Resp_POS => In_S2M.RResp'length,
																				 Data_POS => In_S2M.RData'length, Last_POS => LAST_BITS
		);
	constant B_BIT_VEC      : positive_vector := (ID_POS => In_S2M.BID'length, User_POS => In_S2M.BUser'length, Resp_POS => In_S2M.BResp'length);
	...
	constant MIN_DEPTH      : natural_vector := (
			AW_POS => FRAMES,
			AR_POS => FRAMES,
...
begin
	...
	begin
		...
		begin
			...
			generic map (
				DATA_BITS              => BIT_VEC(i),    -- Data Width
