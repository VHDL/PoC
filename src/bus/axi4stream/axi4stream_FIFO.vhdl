...
architecture rtl of axi4stream_FIFO is
	...
	constant Data_Bits_Vec  : natural_vector := (
		Keep_Pos       => KEEP_BITS,
		Data_Pos       => DATA_BITS,
		Last_Pos       => 1,
		User_Pos       => USER_BITS
...
begin
	...
	begin
		...
		generic map (
			DATA_BITS         => ite(METADATA_IS_DYNAMIC and (USER_BITS > 0), isum(Data_Bits_Vec), isum(Data_Bits_Vec(0 to Last_Pos))),  -- Data Width
		...
		port map (
			...
			-- Writing Interface
			Put            => DataFIFO_put,
			DataIn            => DataFIFO_DataIn(high(Data_Bits_Vec, ite(INCLUDE_META, User_Pos, Last_Pos)) downto low(Data_Bits_Vec, 0)),
			Full           => DataFIFO_Full,
			EmptyState      => open,

			-- Reading Interface
			Got            => DataFIFO_got,
			DataOut           => DataFIFO_DataOut(high(Data_Bits_Vec, ite(INCLUDE_META, User_Pos, Last_Pos)) downto low(Data_Bits_Vec, 0)),
		...
		generic map(
			DATA_BITS          => ite(METADATA_IS_DYNAMIC and (USER_BITS > 0), isum(Data_Bits_Vec), isum(Data_Bits_Vec(0 to Last_Pos))),
			STAGES          => FRAMES
		)
		port map(
			-- Control
	...
	begin
		...
		generic map (
			DATA_BITS          => Meta_In'length,                               -- Data Width
