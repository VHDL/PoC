...
entity dstruct_DoubleEndedQueue is
	generic (
...
architecture rtl of dstruct_DoubleEndedQueue is
	-- Constants
	constant ADDRESS_BITS : natural := log2ceil(MIN_DEPTH);

	-- Signals
	signal combined : std_logic_vector(3 downto 0);
	signal ctrl : std_logic_vector(1 downto 0) := (others => '1');
	signal sub : unsigned(ADDRESS_BITS-1 downto 0) := (others => '0');
	...
	-- Stackpointer
	-- A
	signal stackpointerA : unsigned (ADDRESS_BITS-1 downto 0) := shift_right(to_unsigned(MIN_DEPTH-1,ADDRESS_BITS),1);
	signal weA : std_logic;
	-- B
	signal stackpointerB : unsigned (ADDRESS_BITS-1 downto 0) := shift_right(to_unsigned(MIN_DEPTH-1,ADDRESS_BITS),1) + 1;
	signal weB : std_logic;


	-- ctrl signal for stackpointer operations
	type ctrl_t is (PUSH, POP, NOOP);
	signal ctrlA : ctrl_t := NOOP;
	signal ctrlB : ctrl_t := NOOP;

	-- RAM Signals
	signal adrA : unsigned(ADDRESS_BITS-1 downto 0);
	signal adrB : unsigned(ADDRESS_BITS-1 downto 0);
...
begin
	...
	combined <= PortA_Put & PortA_Got & PortB_Put & PortB_Got;
		...
		if rising_edge(Clock) then
			if Reset = '1' then
				...
				case last_op_ctrl is
					when IDLE =>   last_operation <= last_operation;
					when SET =>    last_operation <= '1';
					when UNSET =>  last_operation <= '0';
				end case;
			end if;
		end if;
	end process;
	...
	begin
		if rising_edge(Clock) then
			if Reset = '1' then
				stackpointerA <= shift_right(to_unsigned(MIN_DEPTH-1,ADDRESS_BITS),1);
			else
				case ctrlA is
					when NOOP => stackpointerA <= stackpointerA;
					when PUSH => stackpointerA <= stackpointerA - 1;
					when POP =>  stackpointerA <= stackpointerA + 1;
				end case;
			end if;
		end if;
	end process;
	...
	begin
		if rising_edge(Clock) then
			if (Reset = '1') then
				stackpointerB <= shift_right(to_unsigned(MIN_DEPTH-1,ADDRESS_BITS),1) + 1;
			else
				case( ctrlB ) is
					when NOOP => stackpointerB <= stackpointerB;
					when PUSH => stackpointerB <= stackpointerB + 1;
					when POP =>  stackpointerB <= stackpointerB - 1;
				end case;
			end if;
		end if;
	end process;
