...
begin
	...
	begin
		assert false report"FLOWCONTROL=" & T_UART_FLOWCONTROL_KIND'image(FLOWCONTROL) & " is currently not supported!" severity failure;
	end generate;
	...
	-- BitClock, Transmitter, Receiver
	-- ===========================================================================
	bclk : entity work.uart_BitClock
		generic map (
	...
	RX : entity work.uart_RX
		...
		port map (
			...
			RX      => UART_RX_sync,
			DataOut      => RXUART_Data,
			Strobe     => RXUART_Strobe,
			ParityError => UART_ParityError
		);
