...
entity fifo_Stage is
	generic (
		...
		LIGHT_WEIGHT : boolean := FALSE -- This option uses half of registers but oscillates between full and empty! Use only when restriction is acceptable (e.g. for Address channel in AXI)
	);
	port (
		-- Control
...
begin
	...
	passthroughGen : if STAGES > 0 generate
		subtype T_slv_d is std_logic_vector(DATA_BITS - 1 downto 0);
	...
	begin
		Full               <= Full_v(0);
		Valid               <= Avail_v(Avail_v'high);
		DataOut                <= do_v(do_v'high);
		di_v(0)           <= DataIn;
		put_v(0)          <= Put;
		...
		LIGHT_WEIGHT_gen : if not LIGHT_WEIGHT generate
			...
			begin
				...
				B <= di_v(i) when rising_edge(Clock) and Avail = '0';
