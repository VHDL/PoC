...
-- ``FillState`` is associated with the read clock domain and outputs the
-- guaranteed number of words available in the FIFO. ``EmptyState`` is associated
...
architecture rtl of fifo_cc_got is
	-- Address Width
	constant ADDRESS_BITS : natural := log2ceilnz(MIN_DEPTH);
...
begin
	...
	Lut_reg_gen : if LUT_SHIFT_LOGIC and MIN_DEPTH <= 32 and DATA_BITS <= 128 and EMPTY_STATE_BITS = 0 and FILL_STATE_BITS = 0 generate
		Lut_fifo : entity work.fifo_Shift
			generic map(
				DATA_BITS    => DATA_BITS,
				MIN_DEPTH => 2 ** ADDRESS_BITS
			)
			port map
			(
				-- Global Control
		...
		assert LUT_SHIFT_LOGIC = false report "PoC.fifo_cc_got.vhdl :: Implementing FIFO in LUT_SHIFT_LOGIC was not possible. Following conditions are not met: 'MIN_DEPTH <= 32 and D_BITS <= 128 and EMPTY_STATE_BITS = 0 and FILL_STATE_BITS = 0'"
		severity warning;
		-----------------------------------------------------------------------------
		-- Pointer Logic
		blkPointer     : block
