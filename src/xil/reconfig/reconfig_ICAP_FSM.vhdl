...
begin
	...
	icap_csb  <= to_sl(not icap_enable) when rising_edge(Clock);
	icap_rw    <= to_sl(icap_read) when rising_edge(Clock);

	-- drive icap data
	icap_in    <= icap_in_r when rising_edge(Clock);
	icap_in_r  <= in_data_swap when in_data_valid = '1' else x"00000000";

	out_data  <= icap_out_swap when rising_edge(Clock);

	-- TODO  detect errors
	status_error <= false;

	-- construct status word
	status(0) <= to_sl(pr_reset) when rising_edge(Clock);
	status(1) <= to_sl(not readback_cnt_rst) when rising_edge(Clock);    -- readback in progress
	status(2) <= to_sl(status_error) when rising_edge(Clock);
	status(3) <= to_sl(cur_state = ready) when rising_edge(Clock);
	status(31 downto 4) <= (others => '0');

	-- edge detection
	in_data_valid_d    <= in_data_valid when rising_edge(Clock);
