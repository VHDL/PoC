...
architecture RandomTransfer of TestController is
	constant MIN_PACKET_SIZE : positive := 1;
	constant MAX_PACKET_SIZE : positive := 20 * DATA_BYTES;
	constant NUM_PACKETS     : positive := 100;

	constant TestName : string          := "TC_RandomTransfer";
...
configuration TC_RandomTransfer of tb_axi4stream_mux is
	for TestHarness
		for TestCtrl : TestController
			use entity work.TestController(RandomTransfer);
		end for;
	end for;
end configuration;
