...
architecture TC_random_mirror of TestControl is
...
begin
	...
	begin
		...
		SetAlertLogName("TC_DeMux_random_mirror") ;
		TranscriptOpen;
		SetTranscriptMirror(TRUE);
		AXI4Stream_SB <= NewID("AXI4Stream_SB", NUMBER_PORTS);
		LOG("PoC.Bus/AXI/AXI4Stream/DeMux Testbench.");
		LOG("Start of Test 3: Random transfer, ADD_MIRROR_MODE => true, ADD_OUTPUT_GLUE => true, ENABLE_REVERSE_USER => false");
	...
	begin
		...
		for index in 0 to NUM_PACKETS -1 loop
			...
			SB    := BytesRV.RandInt(0, NUMBER_PORTS);
			Hit_Vector <= (others => '0');
			if SB /= 0 then
				Hit_Vector(SB -1) <= '1';
			end if;
			...
			for i in 0 to (Bytes +7) / 8 -2 loop
				...
				Keep   := x"FF";
				SData := Keep & Data;
				if SB /= 0 then --Data will be discarded if SB is 0
					Push(AXI4Stream_SB(SB), '0' & mask_data(Data, Keep));
				end if;
			...
			if SB /= 0 then
				Push(AXI4Stream_SB(SB), '1' & mask_data(Data, Keep));
			end if;
...
Configuration TC_DeMux_random_mirror of DeMux_Harness is
	for Harness
		for TestControl_inst : TestControl
			use entity work.TestControl(TC_random_mirror)
			generic map(
				MIN_PACKET_SIZE     => 1,
				MAX_PACKET_SIZE     => 500,
				NUM_PACKETS         => 1500,
