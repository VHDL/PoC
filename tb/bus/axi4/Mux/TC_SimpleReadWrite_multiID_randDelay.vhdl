...
architecture SimpleReadWrite_multiID_randDelay of TestControl is
...
begin
	...
	begin
		...
		begin
			wait until Reset = '0';
			SetUseRandomDelays(AXI4_Manager_Transaction(i));
			...
			WaitForTransaction(AXI4_Manager_Transaction(i));
			wait for 100 ns;
			WaitForBarrier(TestDone);
			wait;
		end process;
	...
	begin
		...
		-------------------------------------
		wait until Reset = '0';
		SetUseRandomDelays(AXI4_Subordinate_Transaction);
		...
		SetModelOptions(AXI4_Subordinate_Transaction, WRITE_RESPONSE_VALID_TIME_OUT, 5000) ;
		SetModelOptions(AXI4_Subordinate_Transaction, READ_DATA_VALID_TIME_OUT, 5000) ;
		WaitForClock(AXI4_Subordinate_Transaction, 2);

		loop
			if WriteAddressV = 0 then
			TryGetWriteAddress(AXI4_Subordinate_Transaction, WriteAddress, Available);
			if Available then
				...
				TryGetWriteData(AXI4_Subordinate_Transaction, WriteData, Available);
				if Available then
			...
			TryGetReadAddress(AXI4_Subordinate_Transaction, ReadAddress, Available);
			if Available then
				...
				SendReadDataAsync(AXI4_Subordinate_Transaction, ReadAddress);
			end if;
