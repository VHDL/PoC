	...
	begin
		...
		for i in 0 to 2**9-1 loop
			...
			for j in 0 to 2**9-1 loop
				...
				cin <= '0';
				WaitForClock(Clock);
				for arch in T_Adder_Architecture loop
					for skip in T_Adder_CarrySkipScheme loop
						...
						AffirmIf(ProcID, expected = to_integer(actual_sum),
							...
							" arch=" & T_Adder_Architecture'image(arch) & " skip=" & T_Adder_CarrySkipScheme'image(skip) & " incl=false"
						);
						...
						AffirmIf(ProcID, expected = to_integer(actual_sum),
							"Output Error: " & integer'image(i) & "+" & integer'image(j) &
							" arch=" & T_Adder_Architecture'image(arch) & " skip=" & T_Adder_CarrySkipScheme'image(skip) & " incl=true"
						);
					end loop;
				end loop;
