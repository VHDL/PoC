...
--                  Gustavo Martin
--
-- Entity:          arith_Divider_TestHarness
--
-- Description:
-- -------------------------------------
-- Test harness for arith_Divider
--
-- License:
-- =============================================================================
...
entity arith_Divider_TestHarness is
end entity;

architecture tb of arith_Divider_TestHarness is
	...
	component arith_Divider_TestController is
		port (
			Clock : in  std_logic;
			Reset : in  std_logic;
			Start : out std_logic;
...
begin
	...
	-- Test Controller
	TestCtrl : arith_Divider_TestController
		port map (
			Clock => Clock,
			Reset => Reset,
			Start => Start,
	...
	genDUTs : for i in 1 to MAX_POW generate
		DUT_SEQU : entity PoC.arith_Divider
			generic map (
				DIVIDEND_BITS => A_BITS,
				DIVISOR_BITS => D_BITS,
				RADIX_EXPONENT  => i
			)
			port map (
		...
		DUT_PIPE : entity PoC.arith_Divider
			generic map (
				DIVIDEND_BITS    => A_BITS,
				DIVISOR_BITS    => D_BITS,
				RADIX_EXPONENT     => i,
			...
			port map (
				...
				Dividend     => A,
				Divisor     => D,
				Quotient     => Q(MAX_POW+i),
				Remainder     => R(MAX_POW+i),
				DivisionByZero     => Z(MAX_POW+i)
			);
	end generate;
