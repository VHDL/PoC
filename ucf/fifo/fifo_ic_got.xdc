...
set_max_delay  -from [get_cells -regexp {gSmall\.regfile.*/.*|gLarge\.ram/gen.ram_reg.*/.*}] \
                 -to [get_cells -regexp {gSmall\.do_reg\[\d+\]|gLarge\.ram/gen\.q_reg\[\d+\]}] \
      -datapath_only [expr "min([get_property period [get_clocks -of_objects [get_pins {Ful_reg/C}]]], [get_property period [get_clocks -of_objects [get_pins {Avl_reg/C}]]])"]
