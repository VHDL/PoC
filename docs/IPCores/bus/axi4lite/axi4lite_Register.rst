...
:term:`AXI4-Lite` communication interface. The register layout is describe by a generic constant call ``Config``. This
constant is constructed by various helper functions as described in the following sections.

The :comp:`axi4lite_Register`

record vector to describe the registers. It was designed to work for all purposes as `Eierlegende Wollmilchsau`.
...
:Mode:        ``latchHighBit_clearOnWrite``
...
1. Each register needs to have a unique name.
...
The ``AXI4Lite-Register`` is capable of creating interrupts. The ``rw_config`` needs to be a latch type to be interrupt
